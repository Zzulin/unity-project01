...
- L13/L17 Controller 均已移除跨系统耦合参数：L13 Inspector 只调体积云，L17 Inspector 只调体积光。RendererFeature 会自动发现当前场景启用的 L13 云，并在内部以 3 次采样、强度 1、对比度 2.8 计算云层对体积光的实时遮挡；低步数透射增加光学深度补偿，耦合实现不再暴露到任何 GameObject。
