...
- LXII 已接入 L13 体积云：`Tools/LXII/Setup L13 VolumeCloud In Game Scene` 会在 `game.unity` 写入 `LXII L13 VolumeCloud Root / LXII Sky Volume Cloud`，使用 LXII 专用材质实例 `Assets/LXII game 整合/Materials/LXII_L13_RaymarchedCloud_Performance.mat`；云盒覆盖天空，默认性能档为 `10 view steps / 0 light steps`，关闭碰撞、阴影和 motion vector；本轮验证为 Unity 编译空闲、Console Error 0、场景/缺引用检查 0 issues，Profiler 快照约 `410 FPS / 2.44 ms / 49 draw calls`。
