...
- `Assets/L17 Volumetric Lighting/L17.unity` 已重做为现代 URP RendererFeature 体积光 Demo：不再手摆 cube 光束，改为低分辨率 froxel/integrated buffer fullscreen 管线，使用 scene depth + 主光 shadowmap + sun direction 做体积积分，加入 blue-noise jitter、temporal reprojection、双边上采样，并在 Bloom / ACES Tonemapping 前合成；场景已简化为房间整体、窗户结构和 2 个室内接光物，主要通过窗框和墙体切出窗光束。
