...
- L17 天空/几何体积散射已统一：移除天空专用 96m 截断和 0.18-0.32 散射降权，天空与几何射线现在使用同一介质密度、消光和米氏相函数，只由真实场景深度提前终止；L18 统一可见距离为 320m、Intensity 8.5，避免山脊处出现“只在地形前有光束、天空无光束”的分类断层。云透射低步数补偿与 2.8 对比度用于恢复空中云缝光幕；L18 启用 temporal accumulation，时间历史开启时不再执行重复的 complementary 第二次积分，保持实时成本。
