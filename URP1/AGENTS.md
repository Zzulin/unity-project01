...
- L17 Controller 已移除全部 L13 云引用和云影调节项，只保留体积光自身参数；RendererFeature 会在当前场景自动发现启用的 L13 云。`Volumetric Shadow Steps / Strength / Contrast` 已迁移到 `L13VolumeCloudController` 的 `Volumetric Light Occlusion` 分组，L18 保留现场值 3 / 1 / 2.1，迁移后画面为 `Assets/Screenshots/L18_cloud_settings_moved_to_L13_20260620.png`。
