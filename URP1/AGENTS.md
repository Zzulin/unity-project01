...
- L13/L17 第二轮耦合修正已完成：L13 视线积分、自阴影和 L17 云遮光均改用世界米制射线，并以每公里换算现有密度/吸收参数，云盒非等比缩放不再改变采样距离单位；Coverage=0 不再粗暴关闭 L17 遮光，而是继续由 WeatherMap 的实际密度决定透射；L13 自阴影与 L17 遮光都采用包含 Detail Erosion 的完整云密度；L17 合成阶段由后处理前改到透明物体前，避免不写深度的 L13 云被低空散射二次覆盖洗白。验证截图：`Assets/Screenshots/L18_coupling_round2_play_20260623.png`。
