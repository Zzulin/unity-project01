...
- L18 当前主光角度固定为场景现值 `(68.00003, 192.52899, 180)`，不再为曝光问题调整方向；针对视线与太阳方向接近时的爆白，L13/L17 增加了默认不影响原场景的前向散射峰值上限，L13 同时限制最终太阳直射项，并启用 ACES。L17 户外介质盒为中心 Y=75/高度 190，顶部 Y=170 与 L13 云底约 Y=160 软衔接，避免云层上方长距离累积。
...
- L13/L17 Controller 均已移除跨系统耦合参数：L13 Inspector 只调体积云，L17 Inspector 只调体积光。RendererFeature 会自动发现当前场景启用的 L13 云，并在内部以 3 次采样、强度 1、对比度 2.1 计算云层对体积光的实时遮挡；耦合实现不再暴露到任何 GameObject。
- L18 色调已从深蓝黑暴风风格调整为现实傍晚参考：后处理取消高对比蓝黑压暗，改为轻微冷色平衡；程序天空降低大气厚度与曝光，环境光和雾提升为明亮蓝灰；主光改为接近中性的暖白，暖色集中在云受光与体积散射。云层当前 Density 3.8 / Coverage 0.2 / Macro Gap 0.72，明显减少覆盖；地面暗部恢复可读。当前预览：`Assets/Screenshots/L18_real_world_warm_blue_final_20260620.png`。
