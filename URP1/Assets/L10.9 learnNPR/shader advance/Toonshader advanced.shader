Shader "Toon/Toonshader advanced"
{
    Properties
    {
        [Main(Base, _, on, off)] _Base ("基础", Float) = 0
        [Tex(Base)] _MainTex ("主纹理", 2D) = "white" { }
        [Tex(Base)] _IlmMap ("IlmMap", 2D) = "white" { }
        ...
        [Sub(Albedo)] _DiffuseCutLocation ("明暗分界位置", Range(0, 1)) = 0.5
        [Sub(Albedo)] _DiffuseCutSmoothness ("明暗过渡", Range(0, 1)) = 0.1
        ...
        [Tex(Hair)] _ShiftMap ("发丝贴图", 2D) = "white" { }
    ...
    SubShader
    {
        ...
        float FaceShadowAttenuation(float3 L, float2 uv, float isConvertFaceCoord)
        {
            // ref: https://zhuanlan.zhihu.com/p/279334552
            float3 frontWS = mul((float3x3)unity_ObjectToWorld,
                lerp(float3(0, 0, 1), float3(0, 1, 0), isConvertFaceCoord));

            // 原神角色模型局部空间和 Unity 世界空间不是对齐的，这里做坐标转换。
            float3 rightWS = mul((float3x3)unity_ObjectToWorld,
                lerp(float3(1, 0, 0), float3(0, 0, -1), isConvertFaceCoord));
        ...
        float3 NPR_Base_Ramp(float NdotL, float rtShadow, float rampRange)
        {
            float3 rampFinal;
            float2 uv;
            uv.x = smoothstep(0, 1, NdotL);
            uv.y = 0.05 + rampRange * 0.4 + _IsNight * 0.5; // 这里采用 openGL 的 uv 坐标约定
        ...
        float NPR_Base_Metallic(float specLayerMask, float3 normalWS)
        {
        ...
        {
            float StepLayerMask = step(specLayerMask, _GlossBlinnMargin);
            float BlinnLayerMask = step(_GlossBlinnMargin, specLayerMask);

            // 模拟视角高光
            float GlossStep = step(_GlossStep, NdotV) * specIntensity * StepLayerMask * _GlossIntensity;

            // Blinn-Phong 高光
            float BlinnStep = step(_BlinnStep, pow(NdotH, specIntensity)) * BlinnLayerMask * _BlinnIntensity;

            float MetalFactor = NPR_Base_Metallic(specLayerMask, normalWS) * _MetalIntensity; // 截断高光
        ...
        float4 frag(v2f i) : SV_Target
        {
            ...
            float rtShadow = mainLight.shadowAttenuation;
            ...
            float shadowAtt = lerp(0.5, 1.0, saturate((nl - _Threshold) * _Hardness))
                * (0.3 + mainLight.shadowAttenuation * mainLight.distanceAttenuation);
            ...
            float3 specFinal = 0;
                ...
                #else
                    specFinal = NPR_Base_Specular(nh, n, baseColor.rgb,
            ...
            float3 rimFinal = 0;
            #if _RIM_ON
                rimFinal = NPR_Base_RimLight(nv, nl, baseColor.rgb * shadowAtt);
            #endif
        ...
        // 截图中能看到 Pass "ShadowCaster" 的结构项，但没有完整代码页。
