    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 BoxProjection(float3 reflDir, float3 worldPos, float4 probePos, float4 boxMin, float4 boxMax)
            {
                ...
                if (probePos.w > 0.0)
                {
                    // Safe division to avoid inf/NaN when a component is near zero.
                    float3 safeDir = sign(reflDir) * max(abs(reflDir), 1e-6);
                    float3 rbmax = (boxMax.xyz - worldPos) / safeDir;
                    float3 rbmin = (boxMin.xyz - worldPos) / safeDir;
                    float3 rbminmax = (safeDir > 0.0) ? rbmax : rbmin;
                    float fa = min(min(rbminmax.x, rbminmax.y), rbminmax.z);
                    float3 hitPos = worldPos + reflDir * fa;
                    reflDir = hitPos - probePos.xyz;
                }
                #endif
                return reflDir;
            }
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                #else
                    reflData = UNITY_SAMPLE_TEXCUBE(unity_SpecCube0, reflDir);
                #endif
