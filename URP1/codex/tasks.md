...
- L17 已完成第三轮天空散射与 shader 热路径修正：移除天空专用 96m 路径与散射降权，天空/几何统一使用同一介质、消光和米氏相函数，几何仅按深度提前结束积分；L18 使用 320m 统一可见距离、Intensity 8.5，云透射对比提高到 2.8。Temporal 固定启用并取消 complementary 第二次完整积分；低深度纹理由 Build/Temporal 复用；L13 三步云透射改为每帧 `128x128` 世界 XZ 缓存，体积每层只做一次 2D 采样；低分辨率降噪为 5 点十字，全分辨率上采样为 3x3，避免 5x5 过量采样。2026-06-24 已恢复 `L17FrustumVolumetricLighting.shader` 的 `_MAIN_LIGHT_SHADOWS_CASCADE` 和 `_SHADOWS_SOFT` 变体；L17 室内窗框/墙体遮挡体积光依赖主光 shadowmap cascade，不能为了清 UnitySkills 的 URP Metal warning 删除这些变体。
...
- L18 已接入动态昼夜色调总控 `L18AtmosphereDirector`：以 `L18 Low Storm Sun` 实际 Euler X 轴做对称插值，X=0/180 为本任务开始前的 GTA5 暖色黄昏参数，X=90 为蓝天白云正午参数；同步主光、程序天空盒、TriLight 环境光/雾色、L13 体积云和 L17 体积光。L13 新增 `RefreshImmediate` 供总控实时推送云参数；编辑态监听主光旋转并按混合值变化刷新。验证截图：`Assets/Screenshots/L18_dynamic_tod_final_noon_auto_20260626.png`、`Assets/Screenshots/L18_dynamic_tod_final_sunset_20260626.png`。
- LXII 已通过 `Tools/LXII/Sync L18 Atmosphere To LXII` 同步 L18 体积云/体积光项目：`game.unity` 保留 Main Camera、妮露玩家、L12 草地，复制 L18 的太阳、昼夜总控、云层、体积光 Bounds、后处理 Volume 与体积光 Controller，并移除旧 Directional Light / LXII 旧云根对象 / 旧 Global Volume。同步脚本负责重绑 L18 材质、主光、RendererData、云和体积光引用，并把 LXII 初始太阳 X 归一到 0 保持黄昏基线。UnitySkills 验证编译空闲、Console Error 0；截图：`Assets/Screenshots/LXII_L18_atmosphere_synced_20260628.png`。
...
## 当前待办
- L18 后续如继续推进，重点转为动态昼夜过渡曲线细化、正午云型美术优化和不同太阳高度下的体积光强度曲线微调。
