...
- L18 已接入动态昼夜色调总控 `L18AtmosphereDirector`：以 `L18 Low Storm Sun` 实际 Euler X 轴插值，X≈20 为本任务开始前的 GTA5 暖色黄昏参数，X=90 为蓝天白云正午参数；同步主光、程序天空盒、TriLight 环境光/雾色、L13 体积云和 L17 体积光。L13 新增 `RefreshImmediate` 供总控实时推送云参数；编辑态监听主光旋转并按混合值变化刷新。验证截图：`Assets/Screenshots/L18_dynamic_tod_final_noon_auto_20260626.png`、`Assets/Screenshots/L18_dynamic_tod_final_sunset_20260626.png`。
